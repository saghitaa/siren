...
import 'services/sos_service.dart'; // Service SOS
import 'services/auth_service.dart';
import 'services/database_service.dart';
import 'settings.dart';
import 'signin.dart'; // Import halaman login
...
class _ResponderDashboardScreenState extends State<ResponderDashboardScreen> {
  ...
  // PENTING: Matikan suara saat keluar dari halaman ini
  @override
  void dispose() {
    _audioPlayer.stop();
  ...
  Future<void> _checkAndPlaySound(List<Report> reports) async {
    ...
    // LOGIKA PERBAIKAN:
    // Hanya bunyikan sirine untuk laporan yang dibuat dalam 24 jam terakhir.
    // Laporan tua yang lupa di-close tidak akan membunyikan sirine.

    bool hasActiveSOS = reports.any((r) {
      final isRecent = now.difference(r.createdAt).inHours < 24;
      return r.type == 'SOS' && r.status == 'SOS_SENT' && isRecent;
    });

    bool hasNewReport = reports.any((r) {
      final isRecent = now.difference(r.createdAt).inHours < 24;
      return r.type != 'SOS' && r.status == 'Belum ditanggapi' && isRecent;
    });

    try {
      if (hasActiveSOS) {
        // Cek agar tidak restart lagu jika sudah playing
        if (_audioPlayer.state != PlayerState.playing) {
          await _audioPlayer.stop();
          await _audioPlayer.setVolume(1.0);
          await _audioPlayer.setReleaseMode(ReleaseMode.loop); // Loop sirine SOS
        ...
        if (_audioPlayer.state != PlayerState.playing) {
          await _audioPlayer.stop();
          await _audioPlayer.setVolume(1.0);
          await _audioPlayer.setReleaseMode(ReleaseMode.stop); // Jangan loop notif biasa
          await _audioPlayer.play(AssetSource('sounds/notif.mp3'));
        }
      } else {
        // JIKA TIDAK ADA YANG AKTIF, PASTIKAN MATI
        await _audioPlayer.stop();
      }
    } catch (e) {
  ...
  void _showMapDialog(
      BuildContext context, double lat, double lng, String title) {
    showDialog(
      context: context,
      builder: (ctx) {
        return Dialog(
                        ...
                        child: Container(
                            ...
                            decoration: ShapeDecoration(
                                ...
                                shape: RoundedRectangleBorder(
                                    borderRadius:
                                    BorderRadius.circular(38835400))))))),
            Positioned(
                left: -84,
                top: -169,
                child: ImageFiltered(
      ...
      child: OutlinedButton.icon(
        onPressed: () async {
          ...
          if (shouldLogout == true) {
            await _audioPlayer.stop(); // Matikan suara dashboard
            await SOSService.instance.stopSiren(); // Matikan suara service global
  ...
  Widget _incidentCard(Report report) {
    ...
    final bool isHandled = report.status == 'Proses' ||
        report.status == 'Selesai' ||
        report.status == 'Menanggapi';
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Container(
                ...
                child: Text(isSOS ? 'SOS ALERT' : report.jenis.toUpperCase(),
                    style: GoogleFonts.orbitron(
                        fontSize: 11,
              ...
              Text(_formatTime(report.createdAt),
                  style: GoogleFonts.instrumentSans(
                      fontSize: 12, color: const Color(0x991A2E35))),
            ],
          ),
          const SizedBox(height: 12),
          Text(report.reportType ?? (isSOS ? 'SOS Darurat' : 'Laporan'),
              style: GoogleFonts.instrumentSans(
                  fontSize: 16,
          ...
          Text(report.description,
              style: GoogleFonts.instrumentSans(
                  fontSize: 14, color: const Color(0xB21A2E35))),
          const SizedBox(height: 12),
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  ...
                  style: GoogleFonts.instrumentSans(
                      fontSize: 12, color: const Color(0xFF1A2E35)),
                  maxLines: 1,
              ...
              if (hasCoordinates)
                GestureDetector(
                  onTap: () => _showMapDialog(
                      context, report.lat!, report.lng!, report.jenis),
                  child: Container(
                    padding:
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: isHandled
                ...
                // 2. Stop Global Service (Jaga-jaga)
                try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        duration: Duration(seconds: 1), // SnackBar cepat
                        content: Text(
                            'Laporan diterima. Status diubah menjadi SEDANG DIPROSES.')));
                _loadReports();
              },
              style: ElevatedButton.styleFrom(
                  backgroundColor: isHandled ? Colors.grey : color,
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12)),
                  padding: const EdgeInsets.symmetric(vertical: 12)),
              child: Text(
                  isHandled ? 'SEDANG DITANGGAPI' : 'TERIMA & TANGGAPI',
                  style: GoogleFonts.orbitron(
                      fontSize: 14,
                      fontWeight: FontWeight.w600,
                      color: Colors.white,
      ...
      decoration: ShapeDecoration(
          ...
          shape: RoundedRectangleBorder(
              ...
              side: const BorderSide(
                  width: 1.16, color: Color(0xCCFFFFFF))),
          shadows: const [
            BoxShadow(
          ...
          Container(
              ...
              decoration: ShapeDecoration(
                  ...
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12),
                      side: const BorderSide(
                          width: 1.16, color: Color(0xB2FFFFFF)))),
              child: Padding(
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('SIREN',
                style: GoogleFonts.orbitron(
                    ...
                    letterSpacing: 4)),
            Text('Smart Integrated Report...',
                style: GoogleFonts.instrumentSans(
                    color: const Color(0x99192D34), fontSize: 10))
          ]),
          const Spacer(),
          Container(
              width: 34,
              ...
              decoration: const ShapeDecoration(
                  ...
                  shape: OvalBorder(
                      side: BorderSide(
                          width: 1.16, color: Color(0x334ADEDE)))),
  ...
  Widget _buildEmptyIncidentCard() {
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(24),
            border: Border.all(
                width: 1.16, color: const Color(0xCCFFFFFF))),
        child: Column(children: [
          ...
          Text('Semua Aman',
              style: GoogleFonts.orbitron(
                  ...
                  color: const Color(0xFF1A2E35))),
          const SizedBox(height: 4),
          Text('Tidak ada laporan darurat aktif saat ini.',
              textAlign: TextAlign.center,
              style: GoogleFonts.instrumentSans(
                  fontSize: 14, color: const Color(0x991A2E35)))
        ]));
  }
              ...
              child: Row(children: [
                Expanded(
                    child: GestureDetector(
                        onTap: () => setState(() => _availabilityIndex = 0),
                        child: Container(
                            height: 36,
                            decoration: BoxDecoration(
                ...
                Expanded(
                    child: GestureDetector(
                        onTap: () => setState(() => _availabilityIndex = 1),
                        child: Container(
                            height: 36,
                            decoration: BoxDecoration(
    ...
    return GestureDetector(
        ...
        child: Container(
            ...
            child: Column(children: [
              ...
              Text(label,
                  style: GoogleFonts.instrumentSans(
                      fontSize: 14, color: const Color(0xFF1A2E35)))
            ])));
  }
